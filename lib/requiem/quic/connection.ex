defmodule Requiem.QUIC.Connection do
  ...
  @spec accept(module, binary, binary) :: {:ok, term} | {:error, :system_error | :not_found}
  def accept(module, scid, odcid) do
    module
    |> to_string()
  ...
  def dgram_send(conn, data) do
    NIF.connection_dgram_send(self(), conn, data)
  end

  @spec stream_send(term, non_neg_integer, binary) ::
          {:ok, non_neg_integer} | {:error, :system_error | :already_closed}
  def stream_send(conn, stream_id, data) do
    NIF.connection_stream_send(self(), conn, stream_id, data)
  end
  ...
  def on_timeout(conn) do
    NIF.connection_on_timeout(self(), conn)
  end
end
